...
class musicdl():
    ...
    def run(self, target_srcs=None, user_interrupt=None):
        while True:
            time.sleep(1)
            ...
            self.config['savedir'] = "downloaded/" + user_input.replace(' ', '')
            self.initializeAllSources()
    ...
    def download(self, songinfos):
        for songinfo in songinfos:
            getattr(self, songinfos[songinfo]['source']).download([songinfos[songinfo]])


    def download2(self, songinfos):
        for songinfo in songinfos:
...
if __name__ == '__main__':
    ...
    config = {'logfilepath': 'musicdl.log', 'savedir': 'downloaded', 'search_size_per_source': 300, 'proxies': {}}
    # target_srcs = [
    #     'baiduFlac', 'kugou', 'kuwo', 'qq', 'qianqian',
    #     'netease', 'migu', 'xiami', 'joox', 'yiting',
    # ]
